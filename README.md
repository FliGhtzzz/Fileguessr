...
- **Elasticsearch 未啟動**: 工具會自動切換為 SQLite 模式，但會失去模糊搜尋與權重排序功能。
- **Ollama 連線失敗**: 請確保 Ollama 桌面應用程式已開啟，且正在執行中。
- **找不到檔案**: 請檢查索引設定中的路徑是否正確，並確保程式有權限存取該路徑。
